...
def load_entln_mat(mat_path):
    ...
    utc_key = next(
        (
            k
            for k in tbl
            if "utc" in k.lower() or "time" in k.lower() or "date" in k.lower()
        ),
        None,
    )
    ...
    if not lat_key or not lon_key or not utc_key:
        raise KeyError(
            f"Could not find lat/lon/UTC fields. Available: {list(tbl.keys())}"
        )
    ...
    print(
        f"  Loaded {len(df):,} pulses  ({df.UTC.min().date()} → {df.UTC.max().date()})"
    )
    return df
...
def find_entln_for_case(case_name, year):
    ...
    candidates = [
        f"{cases_dir}/ENTLN_pulse_{case_name}_struct.mat",
...
def bin_to_grid(df, lats_1d, lons_1d):
    ...
    lat_edges = np.concatenate(
        [[lats_s[0] - dlat], lats_s[:-1] + np.diff(lats_s) / 2, [lats_s[-1] + dlat]]
    )
    lon_edges = np.concatenate(
        [[lons_1d[0] - dlon], lons_1d[:-1] + np.diff(lons_1d) / 2, [lons_1d[-1] + dlon]]
    )
...
def fetch_from_arco(arco_ds, start_str, end_str):
    ...
    plevel_dim = next(
        (d for d in ["level", "pressure_level", "plev"] if d in arco_ds.dims), None
    )
    ...
    if avail_pressure and plevel_dim:
        print(
            f"  Fetching {len(avail_pressure)} pressure vars × {len(ALL_PRESSURE_LEVELS)} levels..."
        )
        flat_vars = {}
...
def append_case(case_name, start_str, end_str, arco_ds):
    ...
    already_there = (
        (existing_times >= case_start) & (existing_times <= case_end)
    ).any()
    if already_there:
    ...
    light_already = (
        (existing_ltimes >= case_start) & (existing_ltimes <= case_end)
    ).any()
    ...
    if entln_path:
        ...
        print(
            f"  WARNING: No ENTLN file found for {case_name} — lightning will be zeros"
        )
    ...
    if light_already:
        ...
        print(
            f"  Already in Lightning.zarr — overwriting in-place with real ENTLN data..."
        )
        indices = np.where(
            (existing_ltimes >= case_start) & (existing_ltimes <= case_end)
        )[0]
        ...
        print(
            f"  Overwritten {len(indices)} timesteps  ({total_lightning} non-zero grid cells) ✓"
        )
    else:
        ...
        print(
            f"  Appending {len(case_times)} lightning timesteps to {LIGHTNING_ZARR}..."
        )
    ...
    for zarr_path, label in [(ERA5_ZARR, "ERA5"), (LIGHTNING_ZARR, "Lightning")]:
        ...
        for name, start, end in MISSING_CASES:
            mask = (times >= pd.Timestamp(start)) & (
                times <= pd.Timestamp(end) + pd.Timedelta(hours=23)
            )
            ...
            if label == "Lightning" and mask.any():
                ...
                status += f"  max_count={max_val:.0f}" + (
                    " ✓" if max_val > 0 else " ✗ ALL ZEROS!"
                )
...
if __name__ == "__main__":
    ...
    print(
        f"ARCO opened. Time range: {pd.Timestamp(arco_ds.time.values[0])} → "
