...
cfg.variable_types = {'lpi', 'KI', 'ds', 'wmax_layer', 'flux_up', 'prec_rate', 'cape2d'}; %'lpi', 'KI', 'ds', 'wmax_layer', 'flux_up', 'prec_rate', 'cape2d'
...
for v = 1:numel(cfg.variable_types)
    ...
    var_cfg = prepare_variable_config(cfg, variable_type);
    ...
    for t = 1:(length(time_edges)-1)
        ...
        process_model_files(var_cfg.variable_type_lower, var_cfg.model_raw_folder, var_cfg.model_out_root, ...
...
function var_cfg = prepare_variable_config(cfg, variable_type)
    var_cfg = cfg;
    ...
    if strcmpi(variable_type, 'KI')
        var_cfg.variable_type_lower = 'KI';
        var_cfg.variable_type_nc = var_cfg.variable_type_lower;
    elseif strcmpi(variable_type, 'ds')
        var_cfg.variable_type_lower = 'ds';
        var_cfg.variable_type_nc = var_cfg.variable_type_lower;
    else
        var_cfg.variable_type_nc = var_cfg.variable_type_lower;
    end

    switch var_cfg.variable_type_lower
        case 'lpi'
        ...
        case 'cape2d'
            ...
            var_cfg.temporal_op = 'mean';
        otherwise
            error('Unsupported variable type: %s', var_cfg.variable_type_lower);
    end

    var_cfg.model_raw_folder = sprintf('%s/%s/Ens/Raw/%s/', ...
        var_cfg.main_path, var_cfg.case_name, var_cfg.variable_type_upper);

    var_cfg.model_out_root = sprintf('%s/proccesed/%s/%s/', ...
        var_cfg.model_raw_folder, var_cfg.grid_label, var_cfg.interval_name);
