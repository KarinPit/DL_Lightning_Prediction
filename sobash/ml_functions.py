...
def get_argparser():
    ...
    parser.add_argument(
        "--flash", type=int, default=10, help="GLM flash count threshold"
    )
    parser.add_argument(
        "--folds",
        nargs="+",
    ...
    parser.add_argument(
        "--ifile",
        help="Read this parquet input file. Otherwise guess which one to read.",
    )
    parser.add_argument(
...
def get_ifiles(args, idir):
    ...
    if model == "HRRR":
        ...
        if args.debug:
            search_str = search_str.replace("HRRRX", "HRRR").replace(
                "20*", debug_replace
            )
        else:
            ...
            search_str += (
                f" {idir}/HRRR_new/grid_data/grid_data_HRRR_d01_202[12]*00-0000.par"
            )
...
def load_df(
    ...
    df = df.rename(
        columns=dict(
            xind="y", yind="x", Date="initialization_time", fhr="forecast_hour"
        )
    )
    ...
    for rptdist in [20, 40]:
        ...
        wbugtimes = slice(
            earliest_valid_time, latest_valid_time - pd.Timedelta(minutes=30)
        )
    ...
    logging.info(
        "update half-G211 y and x coordinates with full-G211 y and x coordinates"
    )
    ...
    if sanity_check:
        ll = ["lat", "lon"]
        xymean = (
            df[["lat", "lon", "lat_y", "lon_y", "y", "x"]].groupby(["y", "x"]).mean()
        )
    ...
    for rptdist in [20, 40]:
        ...
        for t in oldtwin:
            ...
            if any_label_str not in labels:
                continue
            labels_this_twin = [
                f"{r}_{rptdist}km_{t}hr" for r in ["hailone", "wind", "torn"]
            ]
...
def get_glm(
    time_space_window: tuple((float, float)),
    date=None,
    start=None,
    end=None,
    oneGLMfile=True,
...
def predct(i, args, df):
    ...
    yl = yaml.load(
        open(os.path.join(savedmodel_thisfitfold, "config.yaml"), "r"),
        Loader=yaml.Loader,
    )
    if "labels" in yl:
        labels = yl["labels"]
...
def read_csv_files(
    sdate, edate, dataset, members=[str(x) for x in range(1, 11)], columns=None
):
...
def get_flash_pred(
    ...
    oypreds = (
        tmpdir
        / f"Y.{args.flash:03d}+{args.twin}hr.{args.teststart.strftime('%Y%m%d%H')}-{args.testend.strftime('%Y%m%d%H')}.par"
    )
    ...
    if os.path.exists(ifile) and os.path.getmtime(ifile) > os.path.getmtime(oypreds):
        logging.warning(
            f"combined parquet input file {ifile} newer than oypreds {oypreds}; redo oypreds."
        )
        clobber = True
    ...
    if not clobber and os.path.exists(oypreds):
        ...
        logging.warning(
            f"Use valid times [{args.teststart},{args.testend}) for testing"
        )
