...
def plot_cv_indices(cv, X, y, group, ax):
    ...
    for ii, (tr, tt) in enumerate(cv.split(X=X, y=y, groups=group)):
        ...
        img = iax.imshow(
            ...
            aspect="auto",
            cmap=plt.cm.Set1,
            norm=BoundaryNorm([0, 1 - 1 / n_splits, 1.0001], 2),
        )
    ...
    cbar = plt.colorbar(
        ...
        shrink=0.15,
        anchor=(0.94, 0),
        ticks=[(1 - 1 / n_splits) / 2, 1 - 1 / 2 / n_splits],
    ...
    cbar = plt.colorbar(
        img, ax=ax, cax=cax, orientation="horizontal", ticks=np.arange(nclass)
    )
    ...
    if group is not None:
        ...
        iax = ax.inset_axes(rect, transform=ax.transData)
        iax.axis("off")
        img = iax.imshow(group[np.newaxis], aspect="auto", cmap=plt.cm.Paired)
