...
def get_projection_bounds() -> Tuple[float, float, float, float]:
    ...
    pts = G211.transform_points(
        ccrs.PlateCarree(), np.array([ll_lon, ur_lon]), np.array([ll_lat, ur_lat])
    )
...
def getgdf(nlon: int, nlat: int, lon: np.ndarray, lat: np.ndarray) -> gpd.GeoDataFrame:
    ...
    grid = gpd.GeoDataFrame(df.set_index(["y", "x"]), geometry=geometry, crs=crs)
    return grid
...
def getmask(grid: gpd.GeoDataFrame, nlon: int, nlat: int) -> xr.DataArray:
    ...
    """
    world = gpd.read_file(
        "/glade/work/ahijevyc/share/shapeFiles/ne_110m_admin_0_countries"
    )
...
mask = xr.DataArray(
    mask.reshape((nlat, nlon)),
    coords=dict(y=range(nlat), x=range(nlon)),
    dims=["y", "x"],
)
